backaddfor.cind <-
  function(formula, cat.var, data, range = NULL, k = 1, l.s.points = 100, eps = 0.001, repmax = 50, min.p.cat = 1, randnodes = FALSE){
    ...
    cind0 <- cindex.categorization(fit$linear.predictors, survival::Surv(data[,var.names[1]],data[,var.names[2]]))
      ...
      for(rep in 1:5) {
        ...
        if(length(unique(cuts1)) == length(cuts1)) {
          ...
          if (min(table(data$x.cut)) >= min.p.cat) {
            fit <- try(rms::cph(formula.n, data = data))
            cind1 <- cindex.categorization(fit$linear.predictors, survival::Surv(data[,var.names[1]],data[,var.names[2]]))}
          if (cind1 > cind0) {
