  ...
  cpe.boot <- cpe.original  <- vector(length=B)
	...
	for (i in 1:B) {
		...
		if(length(levels(x.cut.boot)) == length(levels(x.cut)) & all(table(x.cut.boot)>1)){
			...
			f.boot <- try(rms::cph(formula.n, data=data.b))
			...
			if("try-error" %in% class(f.boot)){
			  ...
			  cpe.boot[i] <- CPE::phcpe2(coef = f.boot$coefficients, coef.var = f.boot$var, design = stats::model.matrix(f.boot, data = data.b))$CPE  #phcpe(f.boot,CPE.SE=FALSE, out.ties=FALSE)$CPE

				# Original Sample
				# cpe.original[i] <- cpe(f.boot, data.o)
			  cpe.original[i] <- CPE::phcpe2(coef = f.boot$coefficients, coef.var = f.boot$var, design = stats::model.matrix(f.boot, data = data.o))$CPE
			}
		} else {
			cpe.boot[i] <- NA
			cpe.original[i] <- NA
