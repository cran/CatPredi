...
function(formula, cat.var, data, range = NULL, k = 1, l.s.points = 100, eps = 0.001, repmax = 50, min.p.cat = 1, randnodes = FALSE){
  ...
  # Initial nodes
  cuts <- seq(stats::quantile(X, 0.05), stats::quantile(X, 0.95), length = k+2)[-c(1,k+2)]
  cuts0 <- c(min(X)-0.001, cuts, max(X)+0.001)
  data$x.cut <- cut(X, breaks = cuts0, include.lowest=TRUE, right=TRUE)
  formula.n <- stats::update(formula, stats::as.formula(". ~ . + x.cut"))
  fit <- mgcv::gam(formula.n, family = "binomial", data = data)
  auc0 <- compute.empirical.AUC(stats::predict(fit)[Y==1], stats::predict(fit)[Y==0])
  ...
  for(rep in 1:5) {
    ...
    if(length(unique(cuts1)) == length(cuts1)) {
      ...
      if (min(table(data$x.cut)) >= min.p.cat) {
        fit <- mgcv::gam(formula.n, family="binomial", data = data)
        auc1 <- compute.empirical.AUC(stats::predict(fit)[Y==1], stats::predict(fit)[Y==0])}
      if (auc1 > auc0) {
  ...
  while (continue) {
    ...
    data$x.cut <- cut(X, breaks = cuts0, include.lowest=TRUE, right=TRUE)
    fit <- mgcv::gam(formula.n, family = "binomial", data = data)
    list(cuts = cuts0[-c(1,k+2)], auc = auc0, pred = stats::predict(fit, type = "response"))
  }
